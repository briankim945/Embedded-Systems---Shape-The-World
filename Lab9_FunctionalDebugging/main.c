...
unsigned long SW12; 			// input from PF0,4
unsigned long priorSW12; 	// prior input from PF0,4
unsigned long priorLed; 	// input from PF0,4
...
int main(void){  unsigned long i,last,now;
  ...
  while(1){
		SW12 = (GPIO_PORTF_DATA_R&0x11);	// Get inputs of PF4 and PF0
		if (SW12 != 0x11) {		// Check if either PF4 or PF0 is pressed
			Led = GPIO_PORTF_DATA_R;   // read previous
			Led = Led^0x02;            // toggle red LED
		} else {											// If neither PF4 or PF0 is pressed
			Led = Led&~0x02;						// Turn off LED
		}
    GPIO_PORTF_DATA_R = Led;   // output 
    if(
				(
					(SW12 != priorSW12) || (Led != priorLed)	// Chceking if inputs or outputs have changed
				)
				&& (i<50)
		){
    ...
    Delay();
		priorSW12 = SW12;		// Store past SW12 value
		priorLed = Led;			// Store past Led value
  }
}
